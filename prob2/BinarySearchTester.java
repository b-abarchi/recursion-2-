...
public class BinarySearchTester {
	...
	public static int binarySearch( ArrayList<Blob> blobs, Blob bKey1, Blob bKey2 ) {
		int keyCoolness = bKey1.getCoolnessFactor()+bKey2.getCoolnessFactor();
		return binarySearch(blobs, keyCoolness, 0, blobs.size()-1);
	}
	public static int binarySearch( ArrayList<Blob> blobs, int keyCoolness, int low, int high) {
		if(low>high) {
			return -(low-1);
		}
		else {	
				...
				if(mid % 2 == 0) {
					...
					int addedValues = blobs.get(mid).getCoolnessFactor() + blobs.get(mid+1).getCoolnessFactor();
					if(keyCoolness<addedValues) {return binarySearch(blobs, keyCoolness, low, mid-1);}
					else if(keyCoolness>addedValues) {return binarySearch(blobs,keyCoolness, mid+2, high);}
					else {return mid;}
				}
				else {
				mid++;
		...
		if(index>=0) {
			...
			if(indexClosest<blobs.size()) {
				...
				msg = String.format("Search for:%s, Not Found, belongs at:%s at index %d", searchFor, closest, indexClosest);
	                ...
	                String closest = buildTwinString(blobs.get(indexClosest-2),blobs.get(indexClosest-1));
